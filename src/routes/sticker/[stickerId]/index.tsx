...
import { CartLengthContext, StickerContext, totalPriceContext } from "~/lib/store";
...
export default component$(() => {
    ...
    useContextProvider(totalPriceContext, useSignal<number>(sticker.value.price));
    return (
    ...
    return (
        <div class="font-primary">
            ...
            <div class="text-xl text-palette-primary font-medium py-4 px-1">
                <Price currency="$" num={totalPrice.value} numSize="text-2xl" />
            </div>
        </div>
    );
});

export const useStickerForm = routeAction$(async (data) => {
    // Fetch UserSticker in Context
...
export const StickerForm = component$(() => {
    const stickerSize = useSignal("3x3");
    const stickerQuantity = useSignal("1");
    const sticker = useContext(StickerContext);
    const session = useAuthSession();
    const cartLength = useContext(CartLengthContext);
    const stickerform = useStickerForm();
    const totalPrice = useContext(totalPriceContext);

    useTask$(({ track }) => {
        track(async () => {
            cartLength.value = await prisma.cartItem.count();
            const quantity = parseInt(stickerQuantity.value);
            totalPrice.value = quantity * sticker.meta.price;
        });
    });

    return (
        <div class="w-full">
            <Form action={stickerform}>
